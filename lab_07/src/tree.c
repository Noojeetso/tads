...
void
add_size(tree_node_t *tree,
         void *size)
{
    *((size_t *)size) += sizeof(struct tree_node);
}
...
{
    size_t size = 0;
    tree_apply(tree, add_size, &size);
    return size;
}

size_t
get_tree_node_size(void)
{
...
{
    tree_comparisons = new_comparisons;
}
         ...
         int value)
{
    ...
    {
        fputs("Элемент уже находится в ДДП\n", stderr);
         ...
         tree_node_t **tree)
{
    ...
    return tree == NULL ? ERR_NULL_POINTER : EXIT_SUCCESS;
}
         ...
         int value)
{
    ...
    if (find_node(*tree, value) != NULL)
    {
        fputs("Элемент уже находится в АВЛ дереве\n", stderr);
        return ERR_ALREADY_EXISTS;
    }
        ...
        int value)
{
    ...
    if (find_node(*root, value) == NULL)
    {
        fputs("Элемент не найден в АВЛ дереве\n", stderr);
        return ERR_NOT_FOUND;
    }
