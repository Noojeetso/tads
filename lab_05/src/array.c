...
array_queue_t *
new_array_queue(size_t init_size)
{
    ...
    {
        fputs("Ошибка выделения памяти под структуру очереди в виде массива\n", stderr);
        return NULL;
    }
    ...
    if (data == NULL)
    {
        fputs("Ошибка выделения памяти под массив целых чисел для очереди в виде массива\n", stderr);
...
void
free_array_queue(array_queue_t *array_queue)
{
...
int
array_queue_expand(array_queue_t *array_queue)
{
    int *expanded_data;
...
int
array_queue_insert(array_queue_t *array_queue,
                   int value,
                   int index)
{
    int rc;
...
int
array_queue_push(array_queue_t *array_queue,
                 int value)
{
    int rc;
...
int
array_queue_pop(array_queue_t *array_queue,
                int *value)
{
    if (array_queue->size == 0)
    {
        puts("Очередь в виде массива пуста");
...
size_t
get_array_queue_size(array_queue_t *array_queue)
{
    return array_queue->size;
}

int
array_queue_contains(array_queue_t *array_queue,
                     int value)
{
...
void
array_queue_print(array_queue_t *array_queue)
{
    size_t index;
    ...
    puts("Очередь в виде массива, голова слева (1 - задача 1-го типа; 2 - задача 2-го типа):");
    for (size_t i = 0; i < array_queue->size; i++)
    {
...
size_t
get_array_queue_size_bytes(int elements_amount)
{
