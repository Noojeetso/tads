...
void
clear_list_queue(list_queue_t *list_queue)
{
...
void
free_list_queue(list_queue_t *list_queue)
{
...
int
list_queue_insert(list_queue_t *list_queue,
                  int value,
                  ssize_t index)
{
    size_t counter;
...
int
list_queue_push(list_queue_t *list_queue,
                int value)
{
    node_t *new_node;
...
int
list_queue_pop(list_queue_t *list_queue,
               int *value)
{
    node_t *current;
...
void
freed_node_add(freed_nodes_t *freed_nodes,
               void *address)
{
...
int
list_queue_pop_address(list_queue_t *list_queue,
                       int *value,
                       freed_nodes_t *freed_nodes)
{
    node_t *current;
...
void
freed_node_delete(freed_nodes_t *freed_nodes,
                  void *address)
{
    size_t i;
...
int
list_queue_push_address(list_queue_t *list_queue,
                        int value,
                        freed_nodes_t *freed_nodes)
{
    node_t *new_node;
...
int
list_queue_contains(const list_queue_t *list_queue,
                    int value)
{
...
size_t
get_list_queue_size(const list_queue_t *list_queue)
{
    size_t size = 0;
    node_t *curr = list_queue->head;
...
void
freed_nodes_print(freed_nodes_t *freed_nodes)
{
    if (freed_nodes->size == 0)
    {
        puts("Массив освободившихся адресов пуст");
        return;
    }

    puts("Массив освободившихся адресов:");
    printf("№\t| Адрес\n");
    for (size_t i = 0; i < freed_nodes->size; i++)
...
void
list_queue_print(list_queue_t *list_queue)
{
    node_t *curr;
    ...
    if (curr == NULL)
    {
        puts("Очередь в виде списка пуста");
        return;
    }

    puts("Очередь в виде списка, голова слева (1 - задача 1-го типа; 2 - задача 2-го типа):");
    while (curr != NULL)
    {
...
size_t
get_list_queue_size_bytes(int elements_amount)
{
